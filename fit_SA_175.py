...
if __name__ == "__main__":
    ...
    titles = ["Mona Lisa","Johann Sebastian Bach", "The Persistance of Memory", "The kiss"
    ...
    for i in range(0, len(names)):
        ...
        plt.plot(np.arange(start=1500, stop=1500000, step=100), exp_new(np.arange(start=1500, stop=1500000, step=100), m, t, b, d), '--', color="red", label="SA 250 Predict")

        # A straight line of the last value of 250 brush strokes used to compare
        plt.plot(np.arange(start=1000000, stop=1500000, step=1), np.full((500000,1),y_mean[-1]) , '--', color="purple",label = "SA 250 1.000.000")

        # Get xy values for SA 175
        path = "output_dir/"+name+".png/log-SA-175*"
        new_x, y_mean, y_min, y_max = calcXY(path)
        ax.plot(new_x, y_mean, label="SA 175", color="deepskyblue")
        ax.fill_between(new_x,y_min, y_max, alpha=0.2, color="deepskyblue")

        # Create the 175 brush strokes fit
        p0 = (949172,-10244,0.61,505) # start with values near those we expect
        params, cv = scipy.optimize.curve_fit(exp_new, xdata=new_x[1500:], ydata = y_mean[1500:], p0=p0)
        m, t, b, d = params
        SA_fit = exp_new(np.arange(start=1500, stop=1500000, step=100), m, t, b, d)
        plt.plot(np.arange(start=1500, stop=1500000, step=100), exp_new(np.arange(start=1500, stop=1500000, step=100), m, t, b, d), '--', color="blue", label = "SA 175 predict")

        ax.legend()
        plt.xlabel("Evaluations")
        plt.ylabel("Mean squared error")
        plt.ylim(600,limits[i])
